...
function searchByDate(){
	...
	if(from_date.length > 0 && to_date.length > 0){
		...
		window.location = 'cbc_report.php?active=cbc_report&from_date='+from_date+'&to_date='+to_date;
	}else{
               ...
               <div class="row">
                  ...
                  <div class="col-lg-12">
					...
					if(isset($_GET['from_date']) <> '' && isset($_GET['to_date']) <> ''){
						$condition = "where invoice.isCancel = '0' and cbc.date between '$_GET[from_date]' and '$_GET[to_date]'";
					}else{
						$condition = "where invoice.isCancel = '0'";	
					}
					
					$query = $mysqli->query("select * from invoice join cbc on invoice.idNo = cbc.idNo $condition order by invoiceId desc");
					if($query->num_rows > 0){
						$sl = 1;
						$price = 0;
						...
						<tr>
							...
							<td align="center">
                            <a href="cbc_report_details.php?idNo=<?php echo $rows['idNo']; ?>">Report</a>
                            <a href="cbc_report_edit.php?idNo=<?php echo $rows['idNo']; ?>&active=cbc_report">Edit</a>
                              <a href="cbc_report_delete.php?idNo=<?php echo $rows['idNo']; ?>" onclick="return confirm('Are you sure to Remove this')">Remove</a>
                            </td>
						</tr>
						<?php
						$sl++;
