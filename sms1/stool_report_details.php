...
<?php
	...
	//if($addResult){
		$getResult = $mysqli->query("select * from stool join invoice on stool.idNo = invoice.idNo where stool.idNo = '$_GET[idNo]'");
		$metaArray = $getResult->fetch_array();
		?>
        <div class="btn" align="center"><button onClick="window.print()" style="padding:3px 15px; font-weight:bold">Print</button><button onClick="window.location='stool_report.php?active=stool'" style="padding:3px 15px; font-weight:bold">Back</button></div>
        <div id="main_page">
